...
Let $$A_i$$ represent the letter in the $$i^{th}$$ position of sequence $$A$$ before an operation and $$A'_i$$ the letter in the same position after the operation. We will consider the following operations on $$A_i$$:
- a __MATCH__, which keeps $$A_i$$ unchanged, i.e., $$A_i = A'_i$$.
- a __MISMATCH__, which replaces $$A_i$$ by anohter letter of the alphabet, i.e., $$A_i \neq A'_i$$.
- an __INSERTION__, of a letter in $$A_i$$, shifting all remaining letters to the right, i.e., $$\forall j : j >= i, A_j \rightarrow A'_{j+1}$$.
- a __DELETION__, of $$A_i$$, shifting all remaining letters to the left, i.e., $$\forall j : j >= i, A'_{j} \leftarrow A_{j+1}$$.
...
and try a few ways to align them, i.e., to transform $$POINTER$$ into $$PUNTER$$ by applying the above operations.

> _In all examples bellow we "annotate" __MATCHES__ with a "\|", __MISMATCHES__ with a "x", and __GAPS__ with a "-"_
...
- Four __MATCHES__ in positions $$A_4$$ to $$A_7$$
...
All of this seem obvious (and it actually is when we deal with tiny sequences) but this is just a first step to familiarize ourselves with the basic concepts before proceeding to longer and difficult to align sequences.
...
- a __DELETION__ of $$A_1 \times 7$$ (note that, due to the shift, $$A_1$$ iterates over all letters of $$A$$)
...
If you argue that this is a quite silly alignment, I would tend to agree. However, for now, we are just concerned in transforming $$A$$ into $$B$$. All sequences of operations that achieve this goal are good enough.

For the moment we have no way to say if an alignment is better than the other.

# The Optimal Alignment - Some Alignments are Better

From the three examples above it is easy to see that the number of possible alignments between two sequences is huge. In fact, for sequences as small as the ones from our example there are almost 20000 possible alignments <a name="1eton"/>[[1]](#note1).

With so many possible alignments it would be really nice to find the "__best__" alignment of all, a.k.a., the "__optimal__" alignment. To find it we must first define some numerical quantity for each alignment, that we shall call "__score__", and choose as optimal, the one with the highest score.
...
Let's assign one (totally arbitrary) value to each operation:
