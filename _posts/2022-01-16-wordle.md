...
[Information Theory](https://en.wikipedia.org/wiki/Information_theory) is a whole mathematical field created by [Claude Shannon](https://en.wikipedia.org/wiki/Claude_Shannon) in the 1940's. A brilliant mathematical idea, Information Theory was originally developed to study the best way to transmit information over a noisy channel, but quickly became an invaluable tool to solve all sorts of problems, in particular a key tools in the development of modern computer science.

A primer on Information Theory is way out of the scope of this post. However we need to grasp a few basic concepts before going into the game solution.
...
Here $$I(X)$$ stands for the quantity of information associated to the event $$X$$ (e.g. a throw of a dice); $$n$$ represents the number of possible outcomes of the event (e.g. 6); $$x_i$$ represents each individual outcome (e.g. faces 1 through 6); and finally $$P(x_i)$$ is the probability of each individual outcome (e.g. in the case of the dice throw is $$1/6$$ for all outcomes).
...
As a lateral remark note that the quantity Information is usually expressed in $$bits$$, this establishes a fundamental connection with binary coding and the way digital systems encode data, but I digress...

Another important observation is that **_if_** the coin was not fair the amount of information would be less than 1. The following table shows the information of several unfair coins for different probabilities of outcomes:
...
Due to $$P(Heads) = 1 - P(Tails)$$, the remaining rows of the table mirror the rows from above.

Intuitively, it is kind of obvious that if we are sure that a coin always lands on its Tails (first row of the table) we will not obtain any extra information form observing that it indeed landed on its tails, thus Information is zero. Every (almost) sure event (e.g. the sun will rise tomorrow) is not informative.

The corollary of this definition of information - essential for our strategy - is that __the maximum amount of information associated with an event occurs when all the outcomes have the same probability__.
...
__A)__ Both the solution and the guess words belong to the English dictionary. Although obvious, this is a very important point as it limits the universe of solution words: Solutions are always words and not random arrangements of 5 letters. Off course it also limits the universe of guess words. Thus, our goal can be described as: To pick, at any given stage of the game, the _best_ word from a relatively small list (around 5000 words is small for a computer).

__B)__ The game engine randomly draws the solution and every word has the same probability to be drawn. In reality we cannot prove this, but it is a good assumption that simplifies our life.

__C)__ The player gets all the information from the colored clues provided by the game engine.

Assuming these three points, The our _information theoretical_ strategy will be:

1. Choose, from the list of possible words, the most informative one.
2. After receiving the response clues clean up the list of possible words (i.e. remove all words incompatible with the clue).
...
Do you see points A), B) and C) bellow playing a role here?
...
As we already know the formula for information, we can compute how much information we obtain if we play those letters:

$$I(A_1) = - (0.05 \times log_2(0.05) + 0.328 \times log_2(0.328) + 0.622 \times log_2(0.622)) = 1.169$$

$$I(Z_3) = - (0.002 \times log_2(0.002) + 0.016 \times log_2(0.016) + 0.981 \times log_2(0.981)) = 0.151$$
...
However, even if "SEAES" was a word, we probably shouldn't use it. The reason for that is more subtle: When computing the most informative letter for each position we are implicitly assuming that each position is independent, i.e., the occurrence of a given letter in one position doesn't affect the occurrence of letters in other positions. Which is not true! We can verify it with a simple observation: There are almost twice ($${2443 \over 1266} = 1.93$$) as many "E" than "T" in all words, however, after an "A" we find 12.6 ($${151 \over 12} = 12.58$$) times more "T" than "E"! The occurrence of "E" is thus strongly uncorrelated with "A", and we can use this in our favor.

Thus, if we take the word that maximizes the sum of information, _independently_ computed, for each position (which by the way is the word "SAREE") we have no guarantee of obtaining the most informative word (and in fact "SAREE" is not).

Fortunately not everything is lost. If we apply this same approach we can easily find the most informative word.


## The Most Informative Word?

To exemplify how we can extend the concept discussed above to the search of the most informative word lets follow an example: When playing the word "DRINK" what are the possible response clues and how probable are they?
...
The word "TARES" however was unexpected for me. I would expect a word ending in "ES", The "T" in the beginning, however, was a surprise.
...
As expected the most informative word was the best strategy by far. Surprisingly the Random strategy is very close (and I sometimes even better) that the sum of most informative letters, but this analysis is beyond this post.

Is this the best strategy? I honestly don't know. I believe some cleverer approach would be able improve on this.


In any case this seems to be a pretty reasonable approach, being successful for more then 96% of the words. And, most important, it's a neat demonstration of the power and elegance of Information Theoryas a problem solving tool.

<a name="code"/>
# Links and Software

You can reproduce all the numbers and information from this post using [the companion notebook](https://github.com/jaclx5/jaclx5.github.io/blob/master/notebooks/wordle/wordle.ipynb).
